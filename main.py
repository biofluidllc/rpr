...
0.187
        ...
        with c3:
            ...
            x1 = x['Relative Resistance to Flow'].min()
            x1 = round(x1, 2)
            st.success(str(x1) + " rrf") 
        with c2:
            st.text("10 -s Shear Rate RRF")
            y = rrf[rrf['Shear Rate'] < 10]
            y1 = y['Relative Resistance to Flow'].min()
            y1 = round(y1, 2)
            st.warning(str(y1) + " rrf")
        with c1:
            st.text("1 -s Shear Rate RRF")
            z = rrf[rrf['Shear Rate'] < 1]
            z1 = z['Relative Resistance to Flow'].min()
            z1 = round(z1, 2)
            st.error(str(z1) + " rrf")
